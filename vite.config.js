...
import {
  isValidMd5,
  safeDownloadedPath as safeDownloadedPathPure,
  findCachedByMd5 as findCachedByMd5Pure,
} from "./server/pathUtils.js";
...
const CATALOG_API_URL =
  process.env.OPENFORGE_CATALOG_API_URL || "https://staging.openforge.tools";
const CATALOG_OBJECTS_URL =
  process.env.OPENFORGE_CATALOG_OBJECTS_URL ||
...
// only ever being reachable from a trusted network. As a defense-in-depth
// backstop against cross-origin writes/deletes (e.g. a malicious page in
// another tab issuing a fetch() against this origin), state-changing requests
// additionally require the Origin/Referer header (when present) to match the
// request's Host.
...
function openMetadataDb(dir) {
  ...
  try {
    ...
    if (!cols.includes("stl_cached")) {
      db.exec(
        "ALTER TABLE models ADD COLUMN stl_cached INTEGER NOT NULL DEFAULT 0",
      );
    }
  } catch (e) {
...
function downloadedStlPlugin() {
  ...
  const getAllMeta = () =>
    metaDb
      .prepare("SELECT data, stl_cached FROM models ORDER BY file_name")
      .all();
  const getMeta = (sha) =>
    metaDb
      .prepare("SELECT data, stl_cached FROM models WHERE sha = ?")
      .get(sha);
  ...
  const setStlCached = metaDb.prepare(
    "UPDATE models SET stl_cached = ? WHERE sha = ?",
  );
  const withCachedFlag = (row) => ({
    ...JSON.parse(row.data),
    stl_cached: row.stl_cached === 1,
  });
        ...
        if (req.method === "POST") {
          ...
          req.on("end", () => {
            ...
            try {
              ...
              if (shaMatch) {
                try {
                  setStlCached.run(1, shaMatch[1].toLowerCase());
                } catch (e) {
                  /* ignore */
                }
              }
          ...
          if (existsSync(filePath)) {
            ...
            const immutable =
              /^[0-9a-f]{32}\.stl$/i.test(fileName) ||
              fileName.startsWith("thumbs/");
            res.writeHead(200, {
              "Content-Type": mime,
              "Content-Length": data.length,
          ...
          if (shaMatch) {
            try {
              setStlCached.run(0, shaMatch[1].toLowerCase());
            } catch (e) {
              /* ignore */
            }
          }
