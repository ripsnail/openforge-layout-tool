...
// Scan the y≈1.40 shelf
const SHELF_THRESHOLD = 1.3;
const shelfTris = tris.filter(t => t.every(i => Math.abs(vertices[i].y - 1.40) < SHELF_THRESHOLD));
console.log(`Shelf triangles (|y-1.4| < ${SHELF_THRESHOLD}): ${shelfTris.length}`);

const loops = findBoundaryLoops(shelfTris, vertices);
...
for (const tri of shelfTris) { for (const v of tri) {
...
console.log(`Shelf bbox: (${bbMinX.toFixed(1)},${bbMinZ.toFixed(1)}) to (${bbMaxX.toFixed(1)},${bbMaxZ.toFixed(1)})`);
console.log(`Total loops: ${loops.length} (1 perimeter + ${loops.length - 1} inner)\n`);

for (const loop of loops) {
  const area = Math.abs(calculateLoopArea(loop, vertices));
  if (loop === perimeterLoop) {
    console.log(`PERIMETER area=${area.toFixed(1)}`);
    continue;
  }
  ...
  console.log(`area=${area.toFixed(2)} center=(${cx.toFixed(1)},${cz.toFixed(1)}) size=${w.toFixed(1)}x${d.toFixed(1)} maxDim=${Math.max(w,d).toFixed(1)} verts=${loop.length}`);
}
