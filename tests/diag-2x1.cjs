...
console.log(`Top face bbox: (${bbMinX.toFixed(1)},${bbMinZ.toFixed(1)}) to (${bbMaxX.toFixed(1)},${bbMaxZ.toFixed(1)})`);
...
for (const loop of loops) {
  ...
  const nearEdge = distToEdge <= 15 ? 'NEAR-EDGE' : 'interior';
  console.log(`area=${area.toFixed(2)} center=(${cx.toFixed(1)},${cz.toFixed(1)}) size=${w.toFixed(1)}x${d.toFixed(1)} maxDim=${Math.max(w,d).toFixed(1)} dist=${distToEdge.toFixed(1)} ${nearEdge}`);
}
