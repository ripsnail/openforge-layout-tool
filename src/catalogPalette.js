...
function readURLState() {
  ...
  return {
    selectedTags: p.has('req') ? p.getAll('req').flatMap(v => v.split(',').map(decodeURIComponent)).filter(Boolean) : [],
    deniedTags: p.has('deny') ? p.getAll('deny').flatMap(v => v.split(',').map(decodeURIComponent)).filter(Boolean) : [],
...
function writeURLState(selectedTags, deniedTags, activeTab, filterText) {
  ...
  if (selectedTags.length) p.set('req', selectedTags.map(encodeURIComponent).join(','));
  if (deniedTags.length) p.set('deny', deniedTags.map(encodeURIComponent).join(','));
...
export function initPalette(onSelectModel) {
  ...
  hydrateMetadataFromServer().then(({ added, pruned }) => {
    if ((!added || added.length === 0) && (!pruned || pruned.length === 0)) return;
    if (pruned && pruned.length > 0) {
      const prunedSet = new Set(pruned);
      for (let i = downloadedModels.length - 1; i >= 0; i--) {
        if (prunedSet.has(downloadedModels[i]._id)) downloadedModels.splice(i, 1);
      }
  ...
  function renderLocalModels() {
    ...
    document.getElementById('model-count').textContent = `Models: ${allLocalModels.length}`;
  }
  ...
  function renderCatalogSearch() {
    ...
    if (isLoadingCatalog) {
      ...
      resultsDiv.innerHTML = '<div class="no-models">Select tags or search to find parts</div>';
    } else {
      const countEl = document.createElement('div');
      countEl.className = 'catalog-count';
  ...
  function initTagTreeResize(handle, tagsDiv) {
    ...
    if (!isNaN(saved)) {
      document.documentElement.style.setProperty('--tag-tree-height', saved + 'px');
    }
    ...
    handle.addEventListener('mousedown', (e) => {
      ...
      const onMove = (e) => {
        ...
        document.documentElement.style.setProperty('--tag-tree-height', h + 'px');
      };
      const onUp = () => {
        handle.classList.remove('active');
        const current = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--tag-tree-height'));
  ...
  function buildTagCategories(tagsDiv, tagCounts, onTagChange) {
    ...
    for (const cat of TAG_CATEGORIES) {
      ...
      catHeader.addEventListener('click', () => {
        ...
        catHeader.querySelector('.tag-cat-icon').classList.toggle('collapsed', !isHidden);
      });
  ...
  function createModelItem(model) {
    ...
    const overrideIndicator = hasOverride(model.fileName) ? '<span class="override-badge" title="Has manual overrides">⚙</span>' : '';
  ...
  function createCatalogItem(blueprint) {
    ...
    const cachedThumbUrl = getThumbnailUrl(modelInfo._id) || (remoteThumbUrl ? ensureCatalogThumbCached(remoteThumbUrl) : null);
