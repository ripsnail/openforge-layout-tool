...
function parseThemeParts(theme) {
  ...
  if (lastUnderscore >= 0) {
    return {
      set: theme.slice(0, lastUnderscore),
      version: theme.slice(lastUnderscore + 1),
    };
  }
...
export function getEffectiveTextureTags(modelInfo) {
  if (modelInfo?.textureTags && modelInfo.textureTags.length > 0)
    return modelInfo.textureTags;
...
export function resolveTextureColor(textureTags, theme) {
  if (textureTags && textureTags.length > 0) {
    ...
    const versionPick = pickTexture(
      versions.map((t) => t.name),
      true,
    );
    if (versionPick) {
...
export function setTextureOverride(textureTags, name) {
  if (!Array.isArray(textureTags))
    return [{ name, isVersion: false, override: true, tag: `texture|${name}` }];
  const rest = textureTags.filter((t) => !t.override);
  if (!name) return rest;
  return [
    { name, isVersion: false, override: true, tag: `texture|${name}` },
    ...rest,
  ];
}
...
export function isWallBaseTile(modelInfo) {
  ...
  if (ov?.snapBehavior?.acceptsWalls === true && isBaseTile(modelInfo))
    return true;
  return (
    modelInfo.primaryType === "base" &&
